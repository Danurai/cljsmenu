...
(defn hq-phase [plyr]
  ...
  ; Draw 2 cards
  (dotimes (n 2) (card-draw plyr))
  ; Take 4 Resources
  (swap! deck update-in [plyr :resources] + 4)
  ; Ready all exhausted cards
  (swap! deck assoc-in [plyr :decklist] (map #(dissoc % :exhausted) (-> @deck plyr :decklist)))
  ; Pass Initiative
  )

(defn card-discard [plyr idx]
  (swap! deck assoc-in [plyr :decklist] (map #(if (= (:idx %) idx) (assoc % :pos :discard) %)
                              (-> @deck plyr :decklist))))
...
(defn hand-click [plyr idx e]
  (let [card (->> @deck plyr :decklist (filter #(= (:idx %) idx)) first)]
    (when (<= (:cost card 0) (-> @deck plyr :resources))
      (swap! menu assoc :buttons
        (concat 
          (case (:type_code card) 
            "army_unit"  (remove nil? (map #(if (:revealed %) {:title (:name %) :data-action (fn [] (card-deploy plyr (:idx card) (:id %)))}) @planets))
            "attachment" (remove nil? (map #(if (:revealed %) {:title (:name %) :data-action (fn [] (card-deploy plyr (:idx card) (:id %)))}) @planets))
            "event"     [{:title "Play" :data-action #(card-play plyr (:idx card))}]
            "support"   [{:title "Deploy to HQ" :data-action #(card-deploy plyr (:idx card) -1)}]
            nil)
          [{:title "divider"}
...
(defn card-click [plyr idx e]
  (swap! menu assoc :buttons [{:title "Discard" :data-action #(card-discard plyr idx)}
...
(defn cardimgwrap [plyr crd]
    ^{:key (:idx crd)}[:div.whc-card-wrap 
      [:img.whc-card {:class (if (true? (:exhausted crd)) "whc-card-tapped")
                    :src (devimg (:img crd)) 
                    :alt (:name crd)
  ...
  [:div#p1hq
    ...
    [:div
      ...
      [:div#deck.whc-card-wrap
        [:img.whc-card {:src (devimg "/img/cardback.png") :alt "deck" :on-click #(deck-click %)}]
        [:span.whc-cardcount (->> @deck plyr :decklist (filter #(= (:pos %) :deck)) count)]]
      [:div#warlord.whc-card-wrap
        [:img.whc-card {:src (devimg (-> @deck plyr :warlord :img)) :alt (str "Warlord: "  (-> @deck plyr :warlord :name))}]]
      [:div.resources 
        [:div 
        ...
        [:div.m-1
          [:button.btn.btn-block.btn-outline-light.btn-sm
            {:on-click #(hq-phase plyr)} "HQ Phase"]]]
      [:div.whc-card-wrap
        (doall 
          (for [crd (->> @deck plyr :decklist (filter #(= (:pos %) -1)))]
  ...
  (doall (for [p @planets]
      (if (nil? (:winner p))  ; Only show Active Planets
        ^{:key (:id p)}
          ...
          ; P1 deployment
            [:div.col-sm-5
              (for [crd (->> @deck :p1 :decklist (filter #(= (:pos %) (:id p))))]
                (cardimgwrap :p1 crd))
            ; Command Icons  
              [:div.commandl
                (for [n (range (->> @deck :p1 :decklist (filter #(= (:pos %) (:id p))) (map :command_icons) (remove nil?) (reduce +)))]
                  ^{:key n}[:span [:i.fas.fa-gavel.fa-sm.command-icon]])]]
          ; Planets  
            [:div.col-sm-2
              [:div [:img.whc-card-planet {:src (devimg (if (:revealed p) (:img p) "/img/cardback-planet.png")) 
          ...
          ; P2 deployment 
            ...
            ; Command Icons  
              [:div.commandr
                (for [n (range (->> @deck :p2 :decklist (filter #(= (:pos %) (:id p))) (map :command_icons) (remove nil?) (reduce +)))]
                  ^{:key n}[:span [:i.fas.fa-gavel.fa-sm.command-icon]])]
              (for [crd (->> @deck :p2 :decklist (filter #(= (:pos %) (:id p))))]
                (cardimgwrap :p2 crd))]]))))
